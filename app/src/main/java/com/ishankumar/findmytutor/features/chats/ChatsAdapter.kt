...
import com.ishankumar.findmytutor.databinding.ReceiverBinding
import com.ishankumar.findmytutor.databinding.SentBinding
...
class ChatsAdapter(
    ...
    inner class SenderViewHolder(val binding: SentBinding): RecyclerView.ViewHolder(binding.root) {
        fun bindItems(messages: Messages) {
            binding.textSentMessage.text=Decode.decode(messages.message)
        }
    }
    inner class ReceiverViewHolder(val binding: ReceiverBinding): RecyclerView.ViewHolder(binding.root) {
            fun bindItems(messages: Messages) {

                binding.txtReceivedMessage.text=Decode.decode(messages.message)
            }
    }
